...
def _sed_conf_templates(source_folder):
    ...
    sudo(
        f'cd {source_folder} && sed "s/SITENAME/{env.host}/g" {nginx_path} | sudo tee /etc/nginx/sites-available/{env.host}'
    )
    # run(f'sudo ln -s /etc/nginx/sites-available/{env.host} /etc/nginx/sites-enabled/{env.host}')
    sudo(
        f'cd {source_folder} && sed "s/SITENAME/{env.host}/g" {gunicorn_path} | sudo tee /etc/systemd/system/gunicorn-{env.host}'
    )
...
def _start_all_services():
    ...
    sudo(f"service gunicorn-{env.host} restart")
